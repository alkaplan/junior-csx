...
public class SpringPulse extends AbstractSimulation {
	...
	protected void doStep() {
		...
		if(buttonPressed == true) {
			...
			string[0].y = startingY + amplitude * Math.sin(waveTime * period);
			string[0].setCoords();
			waveTime += time;
			if(string[0].y/startingY < 1.01 && string[0].y/startingY > .99 && waveTime > 2*time) {
				throughOrigin++;
			}
			if(throughOrigin == 2) {
				buttonPressed = false;
		...
		if(string[0].y/startingY < 1.01 && string[0].y/startingY > .99) {
	...
	public void initialize() {
		...
		for (int i = 0; i < string.length; i++) {
			...
			string[i].setCoords();
			string[i].color = new Color(255 - colorStep * i, 0, colorStep*i);
		...
		frame.setPreferredMinMax(-.5, 1.5, 5, 15);
	...
	public void reset() {
		...
		control.setValue("maximum x", 1);
		control.setValue("starting y", 10);
		control.setValue("wave amplitude", .25);
		control.setValue("period", 40);
		control.setValue("particle mass", 0.01);
	...
	public static void main(String[] args) {
		SimulationControl.createApp(new SpringPulse());
	}
