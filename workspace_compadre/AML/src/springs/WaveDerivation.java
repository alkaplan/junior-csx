...
public class WaveDerivation extends AbstractSimulation {
	...
	public void runStuff(SpringParticle[] arr, double totalTime, int arrayIndex) {
		...
		if(TvLambdaGraph == true) {
			...
			TvLambdaFrame.setPreferredMinMax(0, 2, 0, 100);
		}
		if(VvLambdaGraph == true) {
	...
	private double findLambda(SpringParticle[] arr) {
		...
		for (int i = arr.length-1; i > 0; i--) {
			...
			if(pastFirstWave == true) {
				if(arr[i].y/tallest > .99 && arr[i].y/tallest < 1.01) {
	...
	public void finishSim() {
		...
		*/
		...
		control.print("Slope T v Lambda: " + TL.m + " r^2: " + correlation(TLambdaX, TLambdaY) + "\n");
		control.print("Slope V v T: " + VT.m + " r^2: " + correlation(VTX, VTY) + "\n");
		control.print("Slope V v Lambda: " + VL.m + " r^2: " + correlation(VLambdaX, VLambdaY) + "\n");
